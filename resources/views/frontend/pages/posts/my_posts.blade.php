            ...
            <div class="flex flex-col gap-4">
                @if(isset($posts))
                    @foreach ($posts as $post)   
                        <div class="w-full border rounded bg-white p-4 shadow-sm">          
                            <div class="flex justify-between items-center mb-2 gap-3">
                                <h1 class=" text-base sm:text-[18px] text-[#DC2D27] " >{{ $post['title'] }}</h1>  
                                
                                <form action="{{ route('posts.destroy', $post['id']) }}" method="POST">
                                    @csrf
                                    @method("DELETE")
                                    <div class="flex items-end justify-end h-full gap-4">
                                        <a href="{{ route('posts.edit', $post['id']) }}" class="shadow  text-black"><i class="fa-solid fa-pen-to-square"></i></a>
                                        <button id="deletePost" data-post-id = '{{ $post['id'] }}' type="submit" class="delete-post text-[#DC2D27] "><i class="fa-solid fa-trash"></i></button>
                                    </div>                                                                        
                                </form>  
                            </div>
                            <a href="{{ route('posts.details', $post['title']) }}"  class="link-post-details">
                                <div class="flex flex-col sm:flex-row w-full justify-between gap-2">
                                    <div>
                                        <img class="w-full sm:w-[150px] rounded object-cover max-h-[150px]" src="{{ (($post->getAvatar(0)) != null) ? asset('uploads/'.$post->getAvatar(0)) : asset('dist/images/preview-1.jpg') }}" alt="">
                                    </div>

                                    <div class="w-full flex flex-col">
                                        <p class="mb-2 text-[14px] text-[#898989] overflow-hidden line-clamp-2">{{ $post['short_description'] }}</p>
                                        <div class="sm:grid flex justify-between gap-1 sm:grid-cols-2 md:flex md:items-center md:justify-start sm:gap-2 lg:gap-8 mb-2 w-full">
                                            <p class="text-xs sm:text-base  flex items-center gap-1 text-[#DC2D27]" >
                                                ...
                                                {{ $post->getCurrencyFormatAttribute() }}
                                            </p>
                                            <p class="text-xs sm:text-base  flex items-center gap-1 text-[#DC2D27]" >
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <p class="text-xs sm:text-base flex items-center gap-1 text-black">
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            ...
                                            <p class="text-xs sm:text-base flex items-center text-black gap-1" >
                                                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                ...
                                                Cách bạn: <span style="color: #DC2D27;"> 2km </span>
                                            </p>
                                        </div>
                                        <p class="mb-auto text-xs sm:text-base" style="color: #868686;">Đã đăng {{ $post->dateDifference() }}</p>                                                                
                                    </div>
                                </div>
                            </a>
                        </div>      
